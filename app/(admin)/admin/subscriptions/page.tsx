                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-[var(--border)]">
                      {orgsWithSubs.map((org) => {
                        const sb = STATUS_BADGE[org.subscription?.status || ""] ?? { variant: "outline" as const, label: org.subscription?.status || "—" };
                        return (
                          <tr key={org.id} className="hover:bg-[var(--muted)]/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-sm">{org.subscription?.plan?.name || "—"}</td>
                            <td className="px-6 py-4"><Badge variant={sb.variant}>{sb.label}</Badge></td>
                            <td className="px-6 py-4 text-sm text-[var(--muted-foreground)]">
                              {org.subscription?.trialEndsAt ? (
                                <span className="flex items-center gap-1">
                                  <Clock className="w-3.5 h-3.5" />{new Date(org.subscription.trialEndsAt).toLocaleDateString()}
                                </span>
                              ) : "—"}
                            </td>
                          </tr>
                    ...
                    return (
                      <div key={org.id} className="p-4 space-y-2">
                        ...
                        <div className="flex items-center justify-between text-xs text-[var(--muted-foreground)]">
                          ...
                          {org.subscription?.trialEndsAt && (
                            <span className="flex items-center gap-1">
                              <Clock className="w-3 h-3" />{new Date(org.subscription.trialEndsAt).toLocaleDateString()}
                            </span>
                          )}
                        </div>
                      </div>
