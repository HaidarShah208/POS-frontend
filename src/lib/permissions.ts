...
export function hasPermission(role: UserRole, permission: Permission, userPermissions?: string[]): boolean {
  ...
  const list = FALLBACK_ROLE_PERMISSIONS[role];
...
export function getNavItemsForRole(role: UserRole, userPermissions?: string[]): NavItem[] {
  if (role === "super_admin") {
    return ADMIN_NAV;
  }
  return ALL_NAV.filter((item) => hasPermission(role, item.permission, userPermissions));
}
